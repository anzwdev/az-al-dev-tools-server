...
namespace AnZwDev.ALTools.Extensions
{
    public static class List_SyntaxNode_Extensions
    {
        ...
        /*
        public static void SortWithTrivia<T>(this List<SyntaxNodeSortInfo<T>> list, IComparer<SyntaxNodeSortInfo<T>> comparer) where T : SyntaxNode
        {
            if (list.Count <= 1)
                return;
            ...
            //check first node
            SyntaxTrivia listLeadingTrivia = default(SyntaxTrivia);
            SyntaxTriviaList leadingTrivias = list[0].Node.GetLeadingTrivia();
            bool listHasLeadingNewLine = ((leadingTrivias.Count > 0) && (leadingTrivias[0].Kind.ConvertToLocalType() == ConvertedSyntaxKind.EndOfLineTrivia));
            if (listHasLeadingNewLine)
            {
                listLeadingTrivia = leadingTrivias[0];
                leadingTrivias = leadingTrivias.RemoveAt(0);
                list[0].Node = list[0].Node.WithLeadingTrivia(leadingTrivias);
            }

            //check last node
            SyntaxTriviaList trailingTrivias = list[list.Count - 1].Node.GetTrailingTrivia();
            bool listHasTrailingNewLine = ((trailingTrivias.Count > 0) && (trailingTrivias[trailingTrivias.Count - 1].Kind.ConvertToLocalType() == ConvertedSyntaxKind.EndOfLineTrivia));
            if (!listHasTrailingNewLine)
            {
                trailingTrivias = trailingTrivias.Add(newLineTrivia);
                list[list.Count - 1].Node = list[list.Count - 1].Node.WithTrailingTrivia(trailingTrivias);
            }

            for (int i = 0; i < (list.Count - 1); i++)
            {
                //check if we should move new line trivia
                trailingTrivias = list[i].Node.GetTrailingTrivia();
                leadingTrivias = list[i + 1].Node.GetLeadingTrivia();
                ...
                if (!trailingNewLine)
                {
                    if (leadingNewLine)
                    {
                        ...
                        leadingTrivias = leadingTrivias.RemoveAt(0);
                        list[i + 1].Node = list[i + 1].Node.WithLeadingTrivia(leadingTrivias);
                    }
                    else
                        trailingTrivias = trailingTrivias.Add(newLineTrivia); //SyntaxFactory.EndOfLine(""));

                    list[i].Node = list[i].Node.WithTrailingTrivia(trailingTrivias);
                }
            }
            ...
            //restore trivias
            if (listHasLeadingNewLine)
                list[0].Node = list[0].Node.WithLeadingTrivia(list[0].Node.GetLeadingTrivia().Insert(0, listLeadingTrivia));
            if (!listHasTrailingNewLine)
            {
                trailingTrivias = list[list.Count - 1].Node.GetTrailingTrivia();
        ...
        */
        ...
        public static void SortWithTrivia<T>(this List<T> list, IComparer<T> comparer) where T : SyntaxNode
        {
            if (list.Count <= 1)
                return;

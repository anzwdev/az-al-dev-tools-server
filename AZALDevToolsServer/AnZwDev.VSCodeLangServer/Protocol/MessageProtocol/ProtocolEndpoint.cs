...
namespace AnZwDev.VSCodeLangServer.Protocol.MessageProtocol
{
    ...
    public class ProtocolEndpoint : IMessageSender
    {
        ...
        {
            return this.SendRequest<object, TResult>(method, null);
        }
        ...
        /// <returns></returns>
        public Task<TResult> SendRequest<TParams, TResult>(string method, TParams requestParams)
        {
            return this.SendRequest<TParams, TResult>(method, requestParams, true);
        }
            ...
            bool waitForResponse)
        {
            ...
            await this.protocolChannel.MessageWriter.WriteRequest<TParams, TResult>(
                method, requestParams, this.currentMessageId);
        ...
        public Task SendNotification<TParams>(
            string method,
            TParams notificationParams)
        {
            // Some requests may still be in the SynchronizationContext queue
            // after the server stops so don't act on those requests because
            // the protocol channel will already be disposed
            ...
            if (!this.InMessageLoopThread)
            {
                ...
                this.SynchronizationContext.Post(
                    async (obj) =>
                    {
                        await this.protocolChannel.MessageWriter.WriteNotification(method, notificationParams);
            ...
            {
                return this.protocolChannel.MessageWriter.WriteNotification(method, notificationParams);
            }
        }
